...
class PortfolioTracker:
    ...
    def sync_eod_from_alpaca(self):
        ...
        Fixes the case where stop/TP orders filled while the agent was
        offline — those exits never make it into portfolio.json, so
        win rate and day_pnl are wrong even though _heal_offset() keeps
        the dollar total correct.
        ...
        try:
            ...
            # Build set of already-recorded (symbol, exit_date) pairs
            existing = set()
            for t in self._trades:
                et = self._trade_exit_et(t)
                if et:
            ...
            # Group buy fills by symbol for entry price lookup
            buy_fills = {
            ...
            # Fallback: current Alpaca positions for avg_entry_price
            try:
            ...
            for o in sell_fills:
                ...
                # Entry price: today's buy fill → Alpaca position → skip
                entry_price = 0.0
                if sym in buy_fills:
                ...
                if entry_price <= 0:
                    logger.info(
                        ...
                        f"no entry price found, skipping (offset covers P&L)"
                    )
                    continue
                ...
                if "stop" in order_type:
                    exit_reason = "Stop loss (recovered)"
                elif "limit" in order_type:
                    exit_reason = "Take profit (recovered)"
                else:
                    exit_reason = "Alpaca EOD close (recovered)"
                ...
                logger.info(
                    ...
                    f"({exit_reason}) @ exit ${exit_price:.2f}"
                )
            ...
            if injected > 0:
                ...
                self._save()
                logger.info(
                    f"[Portfolio] EOD recovery complete — "
                    f"{injected} missing trade(s) injected"
                )
            else:

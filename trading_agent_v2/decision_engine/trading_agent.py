...
from decision_engine.ai_configurator import AIConfigurator
from decision_engine.ticker_universe  import TickerUniverse
from decision_engine.config_log       import ConfigLog
...
class TradingAgent:
    ...
    def __init__(self, config: AgentConfig):
        ...
        # ── AI self-configuration + ticker universe ───────────────────────────
        try:
            import os as _os_ac
            _log_path = _os_ac.path.join(
                _os_ac.path.dirname(_os_ac.path.dirname(_os_ac.path.abspath(__file__))),
                "logs", "ai_config_log.jsonl"
            )
            self._config_log      = ConfigLog(path=_log_path)
            self._ticker_universe = TickerUniverse(
                data_manager=getattr(self, '_data_manager', None),
            ...
            self._ai_configurator = AIConfigurator(
                ...
                config=config,
                portfolio=self._portfolio,
                config_log=self._config_log,
                ticker_universe=self._ticker_universe,
                api_key=_os_ac.environ.get("ANTHROPIC_API_KEY", ""),
            )
            logger.info("[Agent] AIConfigurator + TickerUniverse initialised")
        except Exception as _ai_init_err:
            logger.warning(f"[Agent] AIConfigurator init failed: {_ai_init_err}")
            self._config_log      = None
            self._ticker_universe = None
            self._ai_configurator = None
        import os as _os2
        ...
        self._cycle_count     = 0
        self._symbol_trade_count : dict = {}   # trades per symbol today — churn guard
        self._symbol_loss_count  : dict = {}   # consecutive losses per symbol today — churn guard
    ...
    def start(self):
        ...
        self._thread.start()
        self._notify_status()
        # STARTUP_NEWS_BACKGROUND
        import threading as _sn_th
        def _startup_news():
            try:
                f = getattr(self, '_news_fetcher', None)
                s = getattr(self, '_sentiment_eng', None)
                wl = self.config.watchlist or []
                if f and s and wl:
                    m  = f.fetch_watchlist(wl[:15])
                    sc = s.score_watchlist(m)
                    self.update_news_cache({k: v.to_dict() for k,v in sc.items()})
                    logger.debug(f"[Agent] Startup news warm: {len(sc)} symbols")
            except Exception as _e:
                logger.debug(f"[Agent] Startup news failed: {_e}")
        _sn_th.Thread(target=_startup_news,daemon=True,name="startup-news").start()
    ...
    def _connect(self):
        ...
        # Wire data_manager into TickerUniverse now that it is connected
        if getattr(self, '_ticker_universe', None):
            self._ticker_universe._data_manager = self._data_manager
            self._ticker_universe._portfolio     = self._portfolio
            logger.info('[Agent] TickerUniverse data_manager wired — real scoring enabled')
        ...
        try:
            ...
            try:
                ...
                # UTC_MIDNIGHT_BLOCK1 — use UTC midnight, not local date + Z
                # local date + Z = 5PM yesterday PDT, pulling previous session orders
                from datetime import timezone as _tz
                _midnight_utc = datetime.now(_tz.utc).replace(
                    hour=0, minute=0, second=0, microsecond=0)
                _after = _midnight_utc.strftime("%Y-%m-%dT%H:%M:%SZ")
                ...
                for o in _sells:
                    ...
                    # TRADING_AGENT_SYNC_DRIFT_GUARD block1
                    # V2 stop=0.4% TP=0.8% — nothing legit exceeds 3%
                    _drift1 = abs(exit_price - entry_price) / entry_price if entry_price else 0
                    if _drift1 > 0.03:
                        logger.warning(f"[Agent] Sync SKIPPED {sym}: drift {_drift1:.1%} "
    ...
    def _run_loop(self):
        ...
        while not self._stop_event.is_set():
            if self._pause_event.is_set():
                ...
                # ── AI autonomous evaluation (fires when Claude decides to) ───
                if getattr(self, '_ai_configurator', None):
                    try:
                        _snap = self._ai_configurator.evaluate()
                        if _snap and _snap.decision != 'hold':
                            logger.info(
                                f"[Agent] AI reconfig: {_snap.decision} | "
                                f"trigger={_snap.trigger} | {len(_snap.changes)} change(s)"
                            )
                            if _snap.new_watchlist:
                                logger.info(
                                    f"[Agent] Watchlist updated by AI: "
                                    f"{len(_snap.new_watchlist)} tickers — "
                                    + ', '.join(_snap.new_watchlist[:8])
                                    + ('...' if len(_snap.new_watchlist) > 8 else '')
                                )
                    except Exception as _ai_err:
                        logger.debug(f"[Agent] AI eval error: {_ai_err}")
            time.sleep(10)
    ...
    def _scheduled_scan(self, scan_type: str = "EOD"):
        ...
        if scan_type == "PREMARKET_GAP_SCAN":
            self._run_gap_scan()
            # ── AI pre-market morning brief ──────────────────────────
            if getattr(self, '_ai_configurator', None):
                # PREMARKET_EVAL_BACKGROUND — run in bg so news fetch
                # (4+ min for 15 symbols) doesn't block Cycle #2
                import threading as _pm_th
                def _run_premarket_eval():
                    try:
                        self._ai_configurator._last_reconfig_time  = 0.0
                        self._ai_configurator._last_watchlist_time = 0.0
                        self._ai_configurator._is_premarket_call   = True
                        snap = self._ai_configurator.evaluate(force=True)
                        self._ai_configurator._is_premarket_call   = False
                        if snap:
                            logger.info(
                                f"[Agent] AI PRE-MARKET: {snap.decision.upper()} | "
                                f"{snap.trigger}"
                            )
                    except Exception as _pme:
                        logger.warning(f"[Agent] Premarket eval error: {_pme}")
                _pm_th.Thread(
                    target=_run_premarket_eval, daemon=True,
                    name="premarket-ai-eval"
                ).start()
                logger.info("[Agent] Premarket AI eval running in background")
            return
    ...
    def _sync_today_from_alpaca(self):
        ...
        try:
            ...
            for sym in sells:
                ...
                # TRADING_AGENT_SYNC_DRIFT_GUARD block2
                _drift2 = abs(avg_sell - avg_buy) / avg_buy if avg_buy > 0 else 0
                if _drift2 > 0.03:
                    logger.warning(f"[SyncAlpaca] SKIPPED {sym}: drift {_drift2:.1%} "
                                   f"avg_buy=${avg_buy:.2f} avg_sell=${avg_sell:.2f}")
                    continue
    ...
    def _scan_cycle(self, scan_type: str = 'EOD'):
            ...
            def _auto_news_refresh():
                try:
                    ...
                    if _fetcher and _scorer:
                        ...
                        logger.debug(f"[Agent] Auto news refresh: {len(news_cache)} symbols scored")
        ...
        if scan_type in ("STARTUP", "INTRADAY") and self._hold_overnight:
            ...
            if 9 <= _hour <= 10:  # only in first hour — sell gap and re-enter
                ...
                # ── AI morning brief — runs BEFORE first buy ─────────────
                if getattr(self, '_ticker_universe', None):
                    try:
                        _regime  = "UNKNOWN"
                        _spy_rsi = 50.0
                        if hasattr(self, '_dec_engine') and getattr(self._dec_engine, '_current_regime', None):
                            _r = self._dec_engine._current_regime
                            _regime  = getattr(_r, 'regime',  'UNKNOWN')
                            _spy_rsi = getattr(_r, 'spy_rsi', 50.0)
                        self._ticker_universe.refresh(
                            regime=_regime, spy_rsi=_spy_rsi, force=True
                        )
                        logger.info("[Agent] Universe scored at startup — "
                                    f"{len(self._ticker_universe._scores)} tickers")
                    except Exception as _ue:
                        logger.warning(f"[Agent] Universe refresh failed: {_ue}")

                if getattr(self, '_ai_configurator', None):
                    try:
                        # Reset cooldown so morning brief always fires
                        self._ai_configurator._last_reconfig_time = 0.0
                        _snap = self._ai_configurator.evaluate(force=True)
                        if _snap:
                            logger.info(
                                f"[Agent] AI morning brief: {_snap.decision.upper()} | "
                                f"{_snap.trigger} | {_snap.reasoning[:80]}"
                            )
                            if _snap.new_watchlist:
                                logger.info(
                                    f"[Agent] Morning watchlist ({len(_snap.new_watchlist)}): "
                                    + ", ".join(_snap.new_watchlist[:12])
                                    + ("..." if len(_snap.new_watchlist) > 12 else "")
                                )
                    except Exception as _ae:
                        logger.warning(f"[Agent] AI morning brief failed: {_ae}")
                # ─────────────────────────────────────────────────────────
                    ...
                    try:
                        ...
                        if sig.action == "SELL":
                            ...
                            if "stop" in (sig.reason or "").lower() or "Stop" in (sig.reason or ""):
                                ...
                                self._symbol_loss_count[sig.symbol] = self._symbol_loss_count.get(sig.symbol, 0) + 1
                            ...
                            if order and entry_price > 0:
                                ...
                                if pnl < 0:
                                    ...
                                    self._symbol_loss_count[symbol] = 0  # reset on win
        ...
        if not hasattr(self, '_last_trade_date') or self._last_trade_date != today_date:
            ...
            self._symbol_trade_count = {}
            self._symbol_loss_count  = {}
            ...
            if d.approved and d.action in ("BUY", "SELL"):
                ...
                if d.action == "BUY" and hasattr(self, '_ticker_cd'):
                    ...
                    if not _cd_ok:
                        ...
                        continue
                # ── Churn prevention — max trades + consecutive loss block ──
                if d.action == 'BUY':
                    _max_t = getattr(self.config, 'max_trades_per_symbol', 3)
                    _sym_trades = getattr(self, '_symbol_trade_count', {}).get(d.symbol, 0)
                    if _sym_trades >= _max_t:
                        logger.warning(f"[Agent] {d.symbol} BLOCKED — max {_max_t} trades/day reached ({_sym_trades} done)")
                        continue
                    _sym_losses = getattr(self, '_symbol_loss_count', {}).get(d.symbol, 0)
                    if _sym_losses >= 2:
                        logger.warning(f"[Agent] {d.symbol} BLOCKED — {_sym_losses} consecutive losses today, full-day block")
                        continue
                ...
                if self._executor:
                    ...
                    if order:
                        ...
                        if d.action == "BUY":
                            self._symbol_trade_count[d.symbol] = self._symbol_trade_count.get(d.symbol, 0) + 1
                            logger.debug(f"[Agent] {d.symbol} trades today: {self._symbol_trade_count[d.symbol]}")
        ...
        # ── AI autonomous evaluation (runs after every scan cycle) ───
        if getattr(self, '_ai_configurator', None):
            try:
                _force = getattr(self._ai_configurator, '_force_next_eval', False)
                if _force:
                    self._ai_configurator._force_next_eval = False
                    self._ai_configurator._last_reconfig_time = 0.0
                _ai_snap = self._ai_configurator.evaluate(force=_force)
                if _ai_snap and _ai_snap.decision != 'hold':
                    _wl = _ai_snap.new_watchlist or []
                    logger.info(
                        f"[Agent] AI {_ai_snap.decision.upper()} | "
                        f"trigger={_ai_snap.trigger} | "
                        f"{len(_ai_snap.changes)} change(s)"
                    )
                    if _wl:
                        logger.info(
                            f"[Agent] Watchlist → {len(_wl)} tickers: "
                            + ", ".join(_wl[:10])
                            + ("..." if len(_wl) > 10 else "")
                        )
            except Exception as _ai_e:
                logger.debug(f"[Agent] AI eval error: {_ai_e}")

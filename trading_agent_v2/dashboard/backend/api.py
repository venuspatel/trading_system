...
def get_state():
    ...
    return {
        ...
        "ai_config": (
            {
                "enabled":    _agent._ai_configurator._enabled,
                "eval_count": _agent._ai_configurator._eval_count,
                "summary":    _agent._config_log.summary() if getattr(_agent, "_config_log", None) else {},
            }
            if _agent and getattr(_agent, "_ai_configurator", None) else {"enabled": False}
        ),
        "universe_top10": (
            _agent._ticker_universe.get_scores()[:10]
            if _agent and getattr(_agent, "_ticker_universe", None) else []
        ),
...
# ═══════════════════════════════════════════════════════════════════════
# AI Self-Configuration + Ticker Universe endpoints
# Auto-patched by apply_patch.py
# ═══════════════════════════════════════════════════════════════════════

@app.get("/api/ai_config/status")
def ai_config_status():
    import json as _json, os as _os, datetime as _dt
    ac = getattr(_agent, "_ai_configurator", None) if _agent else None
    cl = getattr(_agent, "_config_log", None) if _agent else None

    def _ts(t):
        if not t: return None
        return _dt.datetime.fromtimestamp(t, tz=_dt.timezone.utc).isoformat()

    # Read summary from log file directly (source of truth)
    summary = cl.summary() if cl else {}
    if not summary.get("total_decisions"):
        _base = _os.path.abspath(_os.path.join(_os.path.dirname(__file__), "..", "..", "logs"))
        _log  = _os.path.join(_base, "ai_config_log.jsonl")
        if _os.path.exists(_log):
            try:
                with open(_log) as _f:
                    lines = [l.strip() for l in _f if l.strip()]
                entries = [_json.loads(l) for l in lines if l]
                if entries:
                    last = entries[-1]
                    summary = {
                        "total_decisions":   len(entries),
                        "expands":  sum(1 for e in entries if e.get("decision")=="expand"),
                        "defends":  sum(1 for e in entries if e.get("decision")=="defend"),
                        "holds":    sum(1 for e in entries if e.get("decision")=="hold"),
                        "watchlist_changes": sum(1 for e in entries if e.get("new_watchlist")),
    ...
    return {
        "enabled":                ac._enabled if ac else False,
        "eval_count":             ac._eval_count if ac else 0,
        "last_reconfig":          _ts(ac._last_reconfig_time) if ac else None,
        "last_watchlist_refresh": _ts(ac._last_watchlist_time) if ac else None,
        "summary":                summary,
        "agent_has_configurator": ac is not None,
    }


@app.get("/api/ai_config/history")
def ai_config_history(limit: int = 50):
    import json as _json, os as _os
    # Primary: use agent's in-memory config_log
    cl = getattr(_agent, "_config_log", None) if _agent else None
    if cl:
        entries = cl.recent(limit)
        if entries:
            return {"entries": entries, "total": len(entries)}
    # Fallback: read log file directly (handles cross-instance case)
    _base = _os.path.abspath(_os.path.join(_os.path.dirname(__file__), "..", "..", "logs"))
    _log  = _os.path.join(_base, "ai_config_log.jsonl")
    if _os.path.exists(_log):
        try:
            ...
            return {"entries": [], "total": 0, "error": str(_e)}
    return {"entries": [], "total": 0, "message": "No log file yet — will appear after first decision"}


@app.post("/api/ai_config/revert/{entry_id}")
def ai_config_revert(entry_id: str):
    if not _agent or not hasattr(_agent, "_ai_configurator"):
        return {"success": False, "error": "AIConfigurator not available"}
    ok = _agent._ai_configurator.revert(entry_id)
    return {"success": ok, "entry_id": entry_id,
            "message": "Reverted successfully" if ok else "Entry not found"}


@app.post("/api/ai_config/evaluate")
def ai_config_force_evaluate():
    """
    Sets a flag for the scan loop to force-evaluate on its next cycle.
    Returns the most recent decision from the log immediately (non-blocking).
    """
    import json as _json, os as _os
    ac = getattr(_agent, "_ai_configurator", None) if _agent else None
    if not ac:
        return {"error": "AIConfigurator not attached to agent — restart V2"}

    # Set flag — scan loop picks it up within 1 scan interval
    ac._force_next_eval = True

    # Return latest logged decision immediately (no blocking)
    _base = _os.path.abspath(_os.path.join(_os.path.dirname(__file__), "..", "..", "logs"))
    _log  = _os.path.join(_base, "ai_config_log.jsonl")
    if _os.path.exists(_log):
        try:
            ...
            if lines:
                ...
                return {
                    "status":       "triggered",
                    "message":      "Force-eval flagged — fires on next scan cycle (within 1 min)",
                    "last_decision": last.get("decision"),
                    "last_trigger":  last.get("trigger"),
                    "last_reasoning":last.get("reasoning","")[:200],
...
@app.get("/api/ai_config/latest")
def ai_config_latest():
    """Return the most recent AI decision from log — always instant, never blocks."""
    import json as _json, os as _os
    _base = _os.path.abspath(_os.path.join(_os.path.dirname(__file__), "..", "..", "logs"))
    _log  = _os.path.join(_base, "ai_config_log.jsonl")
    if not _os.path.exists(_log):
        return {"message": "No decisions logged yet"}
    try:
        with open(_log) as _f:
            lines = [l.strip() for l in _f if l.strip()]
        if not lines:
            return {"message": "Log empty"}
        last = _json.loads(lines[-1])
        return {
            ...
            "watchlist":  last.get("new_watchlist"),
            "timestamp":  last.get("timestamp",""),
            "entry_id":   last.get("entry_id",""),
            "performance":last.get("performance_at_time",{}),
...
@app.get("/api/universe/scores")
def universe_scores():
    if not _agent or not hasattr(_agent, "_ticker_universe"):
        return {"scores": [], "message": "TickerUniverse not initialised — apply trading_agent_patch.py first"}
    tu = _agent._ticker_universe
    import datetime as _dt
    return {
        "scores":         tu.get_scores(),
        "domain_leaders": tu.get_domain_leaders(),
        "total_universe": len(tu._scores),
        "last_refreshed": (
            _dt.datetime.fromtimestamp(tu._last_refresh, tz=_dt.timezone.utc).isoformat()
            if tu._last_refresh > 0 else None
        ),
    }
...
def universe_refresh():
    if not _agent or not hasattr(_agent, "_ticker_universe"):
        return {"error": "TickerUniverse not available"}
    try:
        regime, spy_rsi = "UNKNOWN", 50.0
        if hasattr(_agent, "_dec_engine") and getattr(_agent._dec_engine, "_current_regime", None):
            r = _agent._dec_engine._current_regime
            regime  = getattr(r, "regime", "UNKNOWN")
            spy_rsi = getattr(r, "spy_rsi", 50.0)
        _agent._ticker_universe.refresh(regime=regime, spy_rsi=spy_rsi, force=True)
        scores = _agent._ticker_universe.get_scores()
        return {
            "success": True,
            "scored":  len(scores),
            "top5":    [{"symbol": s["symbol"], "score": s["score"], "streak": s["streak"]}
                        for s in scores[:5]],
        }
    except Exception as e:
        return {"error": str(e)}
...
@app.post("/api/universe/watchlist_refresh")
def watchlist_refresh():
    if not _agent or not hasattr(_agent, "_ai_configurator"):
        return {"error": "AIConfigurator not available"}
    try:
        snap = _agent._ai_configurator.force_watchlist_refresh()
        if snap and snap.new_watchlist:
            return {"success": True, "new_watchlist": snap.new_watchlist,
                    "count": len(snap.new_watchlist), "reasoning": snap.reasoning}
        return {"success": True, "message": "Claude kept existing watchlist", "new_watchlist": None}
    except Exception as e:
        return {"error": str(e)}

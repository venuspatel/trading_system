...
class PortfolioTracker:
    ...
    def sync_eod_from_alpaca(self):
        ...
        Safe to call multiple times — deduplicates on order_id.

        FIX: uses UTC midnight (not ET-date + Z) so we don't pull
        previous-session orders. Dedup on order_id prevents duplicates.
        Does NOT use alpaca_positions as entry fallback (causes phantoms).
        USE_UTC_MIDNIGHT
        SYNC_EOD_DISABLED — disabled 2026-05-22. Creates phantom trades
        by mismatching buy/sell orders across sessions. Agent records
        all trades in real-time via record_trade(). Not needed.
        """
        return  # SYNC_EOD_DISABLED
        ...
        try:
            ...
            # Use UTC midnight today — NOT ET date + Z (that pulls prev session)
            UTC   = timezone.utc
            ET    = timezone(timedelta(hours=-4))
            now_utc   = datetime.now(UTC)
            midnight_utc = now_utc.replace(hour=0, minute=0, second=0, microsecond=0)
            after_str = midnight_utc.strftime("%Y-%m-%dT%H:%M:%SZ")
            ...
            url = (f"{base}/v2/orders"
                   f"?status=closed&after={after_str}&limit=200&direction=asc")
            ...
            if not sell_fills:
                logger.info("[Portfolio] EOD recovery: no sell fills since UTC midnight")
                return

            # Dedup on order_id — more reliable than (symbol, date)
            existing_ids = {
                getattr(t, "order_id", None)
                for t in self._trades
                if getattr(t, "order_id", None)
            }
            # Also keep (symbol, date) fallback for trades without order_id
            existing_sym_date = set()
            for t in self._trades:
                et = self._trade_exit_et(t)
                if et:
                    existing_sym_date.add((t.symbol.upper(), et.strftime("%Y-%m-%d")))

            # Build buy fills map from same UTC-midnight window
            buy_fills = {
            ...
            for o in sell_fills:
                ...
                # Skip if already recorded by order_id
                ...
                # Skip if already recorded by (symbol, date) fallback
                if (sym, filled_date) in existing_sym_date:
                    continue
                ...
                # Entry price MUST come from a same-session buy order.
                # Do NOT fall back to alpaca_positions (old session entry prices
                # cause phantom losses like AVGO $418 entry vs $280 exit = -33%).
                entry_price = 0.0
                if sym in buy_fills:
                    entry_price = float(buy_fills[sym].get("filled_avg_price", 0))
                if entry_price <= 0:
                    logger.info(
                        f"[Portfolio] EOD recovery: {sym} — "
                        f"no same-session buy order found, skipping"
                    )
                    continue

                # Sanity check: skip if price drift > 15% (impossible intraday for stop/TP)
                drift = abs(exit_price - entry_price) / entry_price if entry_price else 0
                if drift > 0.03:  # V2 stop=0.4% TP=0.8%, nothing legit exceeds 3%
                    logger.warning(
                        f"[Portfolio] EOD recovery: {sym} SKIPPED — "
                        f"drift {drift:.1%} too large (entry=${entry_price:.2f} "
                        f"exit=${exit_price:.2f}), likely stale order"
                    )
                    continue
                ...
                existing_sym_date.add((sym, filled_date))
                injected += 1
                logger.info(
                    f"[Portfolio] EOD recovery: injected {sym} "
                    f"pnl=${pnl:+.2f} order_id={order_id}"
                )

            if injected:
                self._save()
                logger.info(f"[Portfolio] EOD recovery: {injected} trade(s) injected")

        except Exception as exc:
            logger.warning(f"[Portfolio] sync_eod_from_alpaca failed: {exc}")
    ...
    def _load(self):
        ...
        try:
            ...
            # ── Phantom trade guard ──────────────────────────────────────
            # If P&L sum < -$50k, portfolio.json has corrupted data from a
            # duplicate-buy event. Reset session stats; preserve trade history.
            _raw_pnl = sum(t.get("pnl", 0) for t in data.get("trades", []))
            if _raw_pnl < -50_000:
                import logging as _log
                _log.getLogger(__name__).warning(
                    f"[Portfolio] Phantom P&L detected: ${_raw_pnl:,.0f}. "
                    f"Resetting day/session stats. Trade history preserved."
                )
                data["day_start_pnl"]  = 0.0
                # Wipe only trades from phantom INTC/duplicate events (pnl < -10000)
                data["trades"] = [t for t in data.get("trades", []) if t.get("pnl", 0) > -10_000]
            # ─────────────────────────────────────────────────────────────────
